...
import com.simulator.distributions.Distributions;
import com.simulator.distributions.Node;
import com.simulator.distributions.RequestOtherStream;
 ...
 * */
	...
	/* Parameters related to generation of docs and workload data files */
	private Distributions distr;
	private static String trafficType;
	private static String dataDir;
	private static String requestStreamFile; 	//the final output file containing the requests
	private static String statisticsFile;    	//some statistics about the generated stream(pop and file size of unique docs)
	private static double other_median_object_size; //In KB, eg. 5 KB
	private static double other_traffic_size;
	private static int  noofDistinctDocs;  	
	private static double web_other_rel;
	private static double other_perc;
	private static long lastObjectEndTime;
	private static double otherZipfSlope;		//Default value: 0.7
	private static double other_redundancy;		//Default value = 0.5
	private static long numTotalRequests;
	private static long workloadSize;
	...
	public SimulationController () throws Exception	{
		...
		/* initializing variables related to Others workload */
		distr = new Distributions();
	}
	...
	public static void initialize_others_params(Properties prop_others) {
		
		try {
			prop_others.load(ClassLoader.getSystemResourceAsStream("others.properties"));
		}
		catch(Exception ex) {
			...
			trafficType = prop_others.getProperty("others.trafficType"); 
			dataDir = prop_others.getProperty("others.dataDir");
	        requestStreamFile = prop_others.getProperty("others.requestStreamFile");
	        statisticsFile = prop_others.getProperty("others.statisticsFile");
	        workloadSize = Long.parseLong(prop_others.getProperty("others.workloadSize"));
	        workloadSize *= 1024*1024;
	        other_perc = Double.parseDouble(prop_others.getProperty("others.other_perc"));
	        other_traffic_size = other_perc * workloadSize;	        		
	        other_median_object_size = Double.parseDouble(prop_others.getProperty("others.other_median_object_size")); 	//In Bytes, 5 KB
	        other_redundancy = Double.parseDouble(prop_others.getProperty("others.other_redundancy"));					//Default value = 0.5
	    	numTotalRequests = (long) (workloadSize/other_median_object_size) + 1;
	        noofDistinctDocs = Math.max((int)((1-other_redundancy) * numTotalRequests),1);  	
	    	web_other_rel = Double.parseDouble(prop_others.getProperty("others.web_other_rel"));
	        lastObjectEndTime = Long.parseLong(prop_others.getProperty("others.lastObjectEndTime"));
	        nextId = Integer.parseInt(prop_others.getProperty("others.nextInt"));
	    	
	        /* 
	    	 * We treat the size values in workload in KBs, so multiply to make workload in GBs 
	    	 * Similarly as the value of median_object_size is in KBs, hence no change to it. 
	    	 */
	        
	    	
	        nextId = Integer.parseInt(prop_others.getProperty("others.nextId"));
	        otherZipfSlope = Double.parseDouble(prop_others.getProperty("others.otherZipfSlope"));
	}
	...
	public static void print_others_parameters() {
		System.out.println("===================================================\n");
		System.out.println("Workload size: "+ other_traffic_size/(1024*1024) +" GB,  Total #requests: " + numTotalRequests + "\n");
		System.out.println("Request Median size = "+ other_median_object_size/(1024) +" MBs\n");
		System.out.println("Total request traffic (1KB/request) = "+ (float)numTotalRequests/1024 +"  MB\n");
		System.out.println("===================================================\n");
		if(numTotalRequests == 0)
		{
	...
	public static void process_file_paths() {
		requestStreamFile = dataDir.concat("/"+requestStreamFile);
		statisticsFile = dataDir.concat("/"+statisticsFile);
	}
	...
	public static void main (String[] args) {
		...
		Properties prop_others = new Properties();
		Distributions distr = new Distributions();

		/* Fetching others properties now */
		initialize_others_params(prop_others);		
		print_others_parameters();
		process_file_paths();

		otherWorkload = new RequestOtherStream(requestStreamFile, statisticsFile, numTotalRequests, other_redundancy, otherZipfSlope,
				nextId, distr, lastObjectEndTime, other_median_object_size, web_other_rel);
