...
import com.simulator.distributions.RequestVideoStream;
 ...
 * */
	...
	private RequestVideoStream videoWorload;
	private static String trafficType;
	private static String dataDir;
	private static String requestStreamFile;
	private static String statisticsFile;
	private static double videoZipfSlope;
	private static double weibullK;
	private static double weibullL;
	private static int numTotalRequests;
	private static long workloadSize;
	private static double video_perc;
	private static double video_redundancy;
	private static int video_pop_distr;
	private static double expected_video_size_mean;
	private static int nextId;
	private static double lastObjectEndTime;
	...
	public SimulationController () throws Exception	{
		...
		/* initializing variables related to video workload */
	...
	public static void initialize_video_params(Properties prop_video) {
		
		try{
			prop_video.load(ClassLoader.getSystemResourceAsStream("video.properties"));
		}
			...
			trafficType = prop_video.getProperty("video.trafficType"); 
			dataDir = prop_video.getProperty("video.dataDir");
	        requestStreamFile = prop_video.getProperty("video.requestStreamFile");
	        statisticsFile = prop_video.getProperty("video.statisticsFile");
	        videoZipfSlope = Double.parseDouble(prop_video.getProperty("video.videoZipfSlope"));
	        weibullK = Double.parseDouble(prop_video.getProperty("video.weibullK").toString());
	        weibullL = Double.parseDouble(prop_video.getProperty("video.weibullL"));
	        workloadSize = Long.parseLong(prop_video.getProperty("video.workloadSize"));
	        workloadSize *= 1024*1024;
	        video_perc = Double.parseDouble(prop_video.getProperty("video.video_perc"));
	        video_redundancy = Double.parseDouble(prop_video.getProperty("video.video_redundancy"));
	        video_pop_distr = Integer.parseInt(prop_video.getProperty("video.video_pop_distr"));
	        expected_video_size_mean = Double.parseDouble(prop_video.getProperty("video.expected_video_size_mean"));
	        expected_video_size_mean *= 1024;
	        nextId = Integer.parseInt(prop_video.getProperty("video.nextId"));
	        lastObjectEndTime = Double.parseDouble(prop_video.getProperty("video.lastObjectEndTime"));
	}
	...
	public static void print_video_parameters() {
		double video_traffic_size = video_perc*workloadSize;
		numTotalRequests = (int)(video_traffic_size/expected_video_size_mean);
		System.out.println("===================================================\n");
		System.out.println("Workload size: "+ workloadSize/(1024*1024) +" GB,  Total Video #requests: " + numTotalRequests + "\n");
		System.out.println("Request Median size = "+ expected_video_size_mean/(1024) +" MBs\n");
	...
	public static void main (String[] args) {
		...
		Properties prop_video = new Properties();
		Distributions distr = new Distributions();

		/* Fetching video properties now */
		initialize_video_params(prop_video);
		print_video_parameters();
		process_file_paths();

		videoWorkload = new RequestVideoStream(requestStreamFile, statisticsFile,
				   videoZipfSlope, numTotalRequests, video_redundancy, weibullK, 
				   weibullL, nextId, distr, lastObjectEndTime, video_pop_distr);
