...
import com.simulator.distributions.RequestP2PStream;
 ...
 * */
	...
	private RequestP2PStream p2pWorkload;
	private static String trafficType;
	private static String dataDir;
	private static String requestStreamFile;
	private static String statisticsFile;
	private static double initMZSlope;
	private static double p2p_redundancy;
	private static long numTotalRequests;
	private static long workloadSize;
	private static double p2p_perc;
	private static int nextId;
	private static boolean fixedP2PSize;
	private static int MZplateau;
	private static double tracesTau;
	private static double tracesLamda;
	private static int torrentInterarrival;
	private static double p2p_median_object_size;
	private static double p2p_fixed_object_size;
	...
	public SimulationController () throws Exception	{
		...
		/* initializing variables related to p2p workload */
	...
	public static void initialize_p2p_params(Properties prop_p2p) {
		
		try{
			prop_p2p.load(ClassLoader.getSystemResourceAsStream("P2P.properties"));
		}
			...
			trafficType = prop_p2p.getProperty("p2p.trafficType"); 
			dataDir = prop_p2p.getProperty("p2p.dataDir");
			requestStreamFile = prop_p2p.getProperty("p2p.requestStreamFile");
			statisticsFile = prop_p2p.getProperty("p2p.statisticsFile");
			initMZSlope = Double.parseDouble(prop_p2p
					.getProperty("p2p.initMZSlope"));
			workloadSize = Long.parseLong(prop_p2p.getProperty("p2p.workloadSize"));
			workloadSize *= 1024 * 1024;
			p2p_perc = Double.parseDouble(prop_p2p.getProperty("p2p.p2p_perc"));
			p2p_redundancy = Double.parseDouble(prop_p2p
					.getProperty("p2p.p2p_redundancy"));
			MZplateau = Integer.parseInt(prop_p2p.getProperty("p2p.MZplateau"));
			tracesTau = Double.parseDouble(prop_p2p.getProperty("p2p.tracesTau"));
			tracesLamda = Double.parseDouble(prop_p2p
					.getProperty("p2p.tracesLamda"));
			torrentInterarrival = Integer.parseInt(prop_p2p
					.getProperty("p2p.torrentInterarrival"));
			p2p_median_object_size = Double.parseDouble(prop_p2p
					.getProperty("p2p.p2p_median_object_size"));
			p2p_fixed_object_size = Double.parseDouble(prop_p2p
					.getProperty("p2p.p2p_fixed_object_size"));
			fixedP2PSize = Boolean.getBoolean(prop_p2p
					.getProperty("p2p.fixedP2PSize"));
			nextId = Integer.parseInt(prop_p2p.getProperty("p2p.nextId"));

			if (p2p_fixed_object_size > 0) {
				p2p_median_object_size = p2p_fixed_object_size * 1024;
				fixedP2PSize = true;
			} else {
				p2p_median_object_size *= 1024;
				fixedP2PSize = false;
			}
	...
	public static void print_p2p_parameters() {
		double p2p_traffic_size = p2p_perc * workloadSize;
		numTotalRequests = (int) (p2p_traffic_size / p2p_median_object_size);
		System.out
				.println("===================================================\n");
		System.out.println("Workload size: " + workloadSize / (1024 * 1024)+ " GB,  Total #requests: " + numTotalRequests + "\n");
		System.out.println("Request Median size = " + p2p_median_object_size / (1024) + " MBs\n");
	...
	public static void main (String[] args) {
		...
		Properties prop_p2p = new Properties();
		Distributions distr = new Distributions();

		/* Fetching p2p properties now */
		initialize_p2p_params(prop_p2p);
		print_p2p_parameters();
		process_file_paths();

		p2pWorkload = new RequestP2PStream(requestStreamFile, statisticsFile,
				initMZSlope, numTotalRequests, p2p_redundancy, MZplateau,
				tracesTau, tracesLamda, torrentInterarrival,
				p2p_median_object_size, nextId, distr, fixedP2PSize);
